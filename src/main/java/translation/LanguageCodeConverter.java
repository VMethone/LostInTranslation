...
public class LanguageCodeConverter {
    ...
    public LanguageCodeConverter(String filename) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (parts.length >= 2) {
                    String name = parts[0].trim();                           // 英文语言名
                    String code = parts[1].trim().toLowerCase(Locale.ROOT);  // 语言代码（如 "en"）
                    languageCodeToLanguage.put(code, name);   // code -> name
                    languageToLanguageCode.put(name, code);   // name -> code
                }
            }
    ...
    public String fromLanguageCode(String code) {
        return languageCodeToLanguage.getOrDefault(
    ...
    public String fromLanguage(String language) {
        return languageToLanguageCode.getOrDefault(language, "Unknown Code");
    }
